...
def run_recbole(model=None, dataset=None, config_file_list=None, config_dict=None, saved=True):
    ...
    # # print(train_data['item_list_index'])
    # input()
    ...
    if os.path.exists('result/{}'.format(config['model'])):
        ...
        os.mkdir('result/{}'.format(config['model']))
    with open('result/{}/{}.txt'.format(config['model'], config['dataset']), mode='a') as filename:
        p = "high_r:{} l:{}-h:{}-b:{}---L2:{}--maxLength:{}-learning_rate:{}-tau:{}-lmd:{}-sim:{}-drop:{}----".format(config['high_r'],config['l_ok'],
                                                                                    config['h_ok'],
                                                                                    config['b_ok'],
                                                                                    config['weight_decay'],
        ...
        filename.write(p)
        filename.write(str(test_result))
        filename.write('\n')  # 换行
